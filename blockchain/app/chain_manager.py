...
def get_url(method):
    return f'{PROXY_URL}/{method}'


def connect(blockchain, block_cls, public_key):
    global _add_block_func, _block_cls, _chain_filename, _blockchain, routes, PROXY_URL
...
class _Connection:
    ...
    @staticmethod
    def setup():
        __chain_json = requests.get(get_url('downloadChain')).text
        if __chain_json:
        ...
        async def update():
            ...
            while True:
                async with request('post', get_url('updateNodes'), data=data) as r:
                    _Connection.nodes = json.loads(await r.text())
                with open(_chain_filename, 'w') as chain:
                    chain.write(str(_blockchain))
                async with request('post', get_url('updateChain'), data={'document': StringIO(str(_blockchain))}) as r:
                    await r.text()
